...
### ⚡ 지도 주변 골프장 검색 API 성능 최적화 및 인메모리 캐싱 도입
* **격자 기반 인메모리 캐시**: `KakaoLocalService` 내에 `ConcurrentHashMap`을 이용한 경량 캐시 레이어를 구축했습니다.
* **위경도 격자화 알고리즘 (`GridKey`)**: 사용자의 미세한 좌표 변화로 인해 외부 카카오 로컬 API가 중복 호출되는 병목을 제거하기 위해, 위경도를 소수점 둘째 자리(약 1.1km 격자 단위)로 반올림 변환하여 캐시 키로 매핑했습니다.
* **캐시 라이프사이클 관리**: 쿼타 보존과 신선도 유지를 위해 **10분 TTL(Time-To-Live)** 정책을 수립하였으며, 캐시 히트 시 외부 통신 없이 **1ms 이내**로 데이터를 즉시 반환합니다.

---

## 🚀 이전 업데이트 (Pre-release v0.9.0 - 2026-06-15)

### 🛡️ 비밀번호 정책 강화 및 백엔드 유효성 검사 도입
* 프론트엔드 우회 가입 공격을 차단하기 위해 `UserService` 가입(`signUp`) 및 수정(`updateProfile`) 시 백엔드 단에서도 동일하게 비밀번호 복잡성(영문/숫자/특수문자 조합 8자 이상) 및 3자 이상 연속된 패턴(1234, asdf) 검사를 통과하도록 강화하고 커스텀 예외 코드를 정의했습니다.

### 🔍 실명/연락처 기반 아이디 찾기 API 추가
* `POST /api/users/find-id` API를 개설하여, 이름과 휴대폰 번호를 대조해 가입된 이메일을 안전하게 마스킹 처리하여 반환하는 비즈니스 로직을 구축했습니다.
* 본인인증 데이터 저장을 위해 `User` 엔티티 내에 실명(`name`) 컬럼을 신설하고 회원가입 시 이를 영속화했습니다.

### 👥 소셜 로그인 회원 실명인증 연동 및 1인 1계정 정책 적용
* 소셜 가입 회원의 실명인증 완료 시 정보를 갱신하고 중복 가입을 확인하는 `POST /api/users/{id}/social-verify` API를 구축했습니다.
* 회원가입 및 소셜 인증 시 이미 동일한 번호로 가입된 다른 사용자(ID 또는 이메일 불일치)가 존재한다면 `PHONE_ALREADY_EXISTS` 예외를 리턴하여 다중 계정 생성을 완전히 차단합니다.
* 임시 번호(`000-0000-0000`) 유저가 복수 존재하는 상태에서 중복 검사 시 발생하는 `NonUniqueResultException`을 방지하도록 쿼리 조회 타입을 `List` 기반으로 리팩토링했습니다.
